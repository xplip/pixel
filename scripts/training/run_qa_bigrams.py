...
from pixel.data.rendering.pangocairo_renderer_bigrams import PangoCairoBigramsRenderer as PangoCairoBigramsRenderer
from pixel.utils.question_answering_bigrams import postprocess_qa_predictions
    ...
    if modality == Modality.IMAGE:
        ...
        def prepare_train_image_features(examples):
            ...
            encodings = [
                ...
                for q, c, is_rtl in zip(examples[question_column_name], examples[context_column_name], 
                                        [reading_direction_is_rtl[ex.split('-')[0]] for ex in examples['id']])
            ]
            sample_mapping = []
            full_encodings = []
            ...
            processed_examples["word_to_patch_maps"] = [encoding.word_patch_map for encoding in full_encodings] # rely on `word_patch_map` for this 
            
            # Include a tuple for offsets from word_start to answer_start. This is !=0 when answer is not at the beginning of a word
            # e.g. caused py leading brackets, etc.
            processed_examples['answer_start_offset'] = []

            # Let's label those examples!
            processed_examples["start_positions"] = []
            processed_examples["end_positions"] = []

            for i, word_to_patch_map in enumerate(processed_examples["word_to_patch_maps"]): 
                ...
                # Check if sample is RTL 
                is_rtl = word_to_patch_map['text_b']['is_rtl']
                sentence_key = "sentence_len_rtl" if is_rtl else 'sentence_len_ltr'
                # If no answers are given, set the cls_index as answer.
                if len(answers["answer_start"]) == 0:
                    processed_examples["start_positions"].append(cls_index)
                    processed_examples["end_positions"].append(cls_index)
                else:
                    # Start/end patch index of the answer in the text.
                    ...
                    # Start patch index of the current span in the text.
                    word_index_answer_start = processor.find_word_index_from_sentence_len(
                        dictionary=word_to_patch_map["full_text_b"],
                        sentence_len=start_char,
                        rtl=is_rtl
                    ) 
                    
                    # Save overview of offset differences 
                    offset_diff = word_to_patch_map['full_text_b']['word_order_on_canvas'][word_index_answer_start][sentence_key] - start_char
                    processed_examples['answer_start_offset'].append(f"{offset_diff}_{is_rtl}")

                    num_words_in_answer = len(processor.whitespace_tokenize(answers[text_key][0]))
                    if is_rtl:
                        word_index_answer_start -= 1
                        word_index_answer_end = word_index_answer_start - num_words_in_answer + 1
                        word_index_answer_start, word_index_answer_end = word_index_answer_end, word_index_answer_start
                    else:
                        word_index_answer_start += 1
                        word_index_answer_end = word_index_answer_start + num_words_in_answer - 1 # UP TO AND INCLUDING 

                    # Rely on `sliced_word_patch_map_b` to know if answer is in overflow encoding
                    text_b_key = 'sliced_word_patch_map_b' if 'sliced_word_patch_map_b' in word_to_patch_map else 'text_b'
                    # Check if entire answer text is within sequence or not 
                    if not set([word_index_answer_start, word_index_answer_end]).issubset(
                        word_to_patch_map[text_b_key]['word_order_on_canvas'].keys()
                    ):
                        processed_examples["start_positions"].append(cls_index)
                        processed_examples["end_positions"].append(cls_index)
                    else:
                        # If overflow encoding, we need a mapping to where the answer is in the overflow sequence,
                        # i.e. going from `sliced_word_patch_map_b` to `text_b`
                        # `text_b` is where the text is the sample-specific encoding. 
                        overflow_map = {k:v for k,v in zip(
                            word_to_patch_map[text_b_key]['word_order_on_canvas'],
                            word_to_patch_map['text_b']['word_order_on_canvas'], 
                            )}
                        # If last word in answer is also last word in sequence, append index of `first_patch_after_last_word`
                        
                        if word_index_answer_end == word_to_patch_map[text_b_key]['word_list_truncation_index']:
                            processed_examples["start_positions"].append(
                                word_to_patch_map['text_b']['word_order_on_canvas'][overflow_map[word_index_answer_start]]['start_patch_index']
                            ) 
                            processed_examples["end_positions"].append(
                                word_to_patch_map['text_b']['first_patch_after_last_word'] - 1
                            )
                            ...
                            processed_examples["end_positions"].append(
                                word_to_patch_map['text_b']['word_order_on_canvas'][overflow_map[word_index_answer_end + 1]]['start_patch_index'] - 1 
                            )
        ...
        def prepare_validation_image_features(examples):
            ...
            # The offset mappings will give us a map from token to character position in the original context. This will
            # help us compute the start_positions and end_positions.
            processed_examples["offset_mapping"] = []
            ...
            for i, enc in enumerate(full_encodings):
                ...
                processed_examples["offset_mapping"].append(
                    [f"""
                        {enc.word_patch_map['text_b']['word_order_on_canvas'][i]['start_patch_index']}
                        {enc.word_patch_map['text_b']['word_order_on_canvas'][i]['word']}
                        {enc.word_patch_map['text_b']['word_order_on_canvas'][i]['word_len']}
                        {enc.word_patch_map['text_b']['word_order_on_canvas'][i]['sentence_len_ltr']}
                        {enc.word_patch_map['text_b']['word_order_on_canvas'][i]['sentence_len_rtl']}
                        """
                        for i in range(enc.word_patch_map['text_b']['word_list_truncation_index']+1)] + [
                            str(enc.word_patch_map['text_b']['first_patch_after_last_word'])
                            ] + [
                                str(enc.word_patch_map['text_b']['is_rtl'])
                            ]
                )
                ...
                features = Features(
                    {
                        ...
                        "offset_mapping": Sequence(Value(dtype="string", id=None)),
